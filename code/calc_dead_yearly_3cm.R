calc_dead <- function(){
  ...
  load(file="../../drought12.Rdata")
  load(file="../../drought13.Rdata")
  load(file="../../drought14.Rdata")
  load(file="../../drought15.Rdata")
  ...
  results_k <- foreach(i=inputs, .combine = rbind,.packages = c('raster','rgeos','tidyr','dplyr'), .errorhandling="remove") %dopar% {
    ...
    pmerge <- pmerge %>% 
      ...
      dplyr::mutate(live.ratio = TPH_GE_3/sum(TPH_GE_3)) %>% 
      dplyr::mutate(relNO = tot_NO*live.ratio) %>% 
      dplyr::mutate(BPH_abs = BPH_GE_3_CRM*(900/10000)) %>% 
      dplyr::mutate(BM_tree_kg = BPH_GE_3_CRM/TPH_GE_3) 
    ...
    for(l in 1:nrow(pmerge)) {
      if(pmerge[l,"TPH_GE_3"]*(900/10000)<pmerge[l,"relNO"]) {
    ...
    pmerge$trunc <- ifelse(pmerge$D_BM_kg==pmerge$BPH_GE_3_CRM*(900/10000) & pmerge$D_BM_kg!=0, 1,0)
