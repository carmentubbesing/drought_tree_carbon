...
counties <- spTransform(counties, crs(spdf))
...
for(i in 47:length(counties)){
  ...
  plot(spdf, add = T, pch = ".", col = i)
}
```

# Reformat live biomass from Imperial county 
```{r}
load("~/drought_tree_carbon/drought_tree_carbon/results/Counties/Results_spatial/live_lemma_spdf_Imperial.Rdata")
spdf <- live_lemma_spdf
lay <- "Imperial"
spdf@data <- spdf@data %>% 
  mutate(BM_live_2012_kg = BPH_GE_25_CRM*.09) %>% 
  mutate(pixel_key = as.factor(seq(1, nrow(spdf), by = 1))) %>% 
  mutate(Percent_Mortality_Biomass = 0) %>% 
  mutate(D_BM_kg = 0) %>% 
  dplyr::select(pixel_key, x, y, D_BM_kg, BM_live_2012_kg, Percent_Mortality_Biomass)
